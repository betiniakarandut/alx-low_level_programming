...
{
	/**
	 * this is going to be a totalsum of return values
	 */
	int totalsum = 0;
	 ...
	 * of wildcmp() to totalsum
	 * add the return value of bandersnatch() to totalsum
	...
	if (*s2 == '*')
	{
		...
		if (*s2 == '\0')
			return (1);
		if (*s1 == *s2)
			totalsum += wildcmp(s1 + 1, s2 + 1);
		totalsum += bandersnatch(s1 + 1, s2);
		return (!!totalsum);
	}
	return (0);
}
